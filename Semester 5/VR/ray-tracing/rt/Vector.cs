...
namespace rt
{
    public class Vector(double x, double y, double z)
    {
        ...
        // Scalar product
        // used for angles and light intensity
        ...
        {
            // v' = 2 * (u . v) * u + (w^2 - u . u) * v + 2w * (u x v)
            // v = this (current vector)
            // q = (w, u) (w = scalar, u = vector)

            // Vector part
            Vector u = new Vector(q.X, q.Y, q.Z);
            
            // Scalar part
            double w = q.W;

            // Cross (vectorial) product (u x v)
            Vector crossProduct = u ^ this;
            
            // Dot (scalar) product (u . v)
            double dotProduct = u * this;

            // Apply formula
            Vector rotated = (u * (2.0 * dotProduct)) + (this * (w * w - u.Length2())) + (crossProduct * (2.0 * w));
