...
def generate_random_grades(number_of_grades: int, students_data, disciplines_data):
    generated_grades = []
    ...
    for _ in range(number_of_grades):
        ...
        generated_grades.append(current_grade)
    return generated_grades


def test_generate_random_grades():
    students_data = [Student(1, "Alice"), Student(2, "Bob")]
    disciplines_data = [Discipline(101, "Computer Arhitecture"), Discipline(102, "Fundamentals of programming")]
    assert len(generate_random_grades(2, students_data, disciplines_data)) == 2
