...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // If no errors, update destination in database
    if(empty($errors)) {
        $sql = "UPDATE destinations 
                SET country_id = ?, location_name = ?, description = ?, tourist_targets = ?, cost_per_day = ?
                WHERE destination_id = ?";
                
        if($stmt = mysqli_prepare($conn, $sql)) {
            mysqli_stmt_bind_param($stmt, "isssdi", $country_id, $location_name, $description, $tourist_targets, $cost_per_day, $destination_id);
...
<body>
    ...
    <div class="container">
        <div class="card">
            ...
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . "?id=" . $destination_id; ?>" method="post" id="editDestinationForm">
