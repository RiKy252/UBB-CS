...
public class MainWindowController {
    ...
    @FXML
    private TableView<Pair<String, Pair<List<String>, IStmt>>> proceduresTableView;

    @FXML
    private TableColumn<Pair<String, Pair<List<String>, IStmt>>, String> procedureTableAnthemColumn;

    @FXML
    private TableColumn<Pair<String, Pair<List<String>, IStmt>>, String> procedureTableBodyColumn;
    ...
    public void refresh() {
        ...
        if (controller.getPrograms().size() > 0) {
            ...
            procTable = controller.getPrograms().getFirst().getProcTable();
        }
        ...
        if (procTable != null) {
            procTable.getProcedureTable().forEach((k, v) -> proceduresTableView.getItems().add(new Pair<>(k, v)));
        }
        ...
        try {
            ...
            this.proceduresTableView.refresh();
        }
    }
    ...
    public void initialize() {
        ...
        this.procedureTableAnthemColumn.setCellValueFactory(p -> new SimpleObjectProperty<>(p.getValue().getKey().toString()));
        this.procedureTableBodyColumn.setCellValueFactory(p -> new SimpleObjectProperty<>(p.getValue().getValue().toString()));
        this.refresh();
