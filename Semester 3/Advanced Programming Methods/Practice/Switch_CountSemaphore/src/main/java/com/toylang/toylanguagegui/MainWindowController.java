...
public class MainWindowController {
    ...
    @FXML
    private TableColumn<Pair<Integer, Pair<Integer, List<Integer>>>, String> semaphoreValueColumn;

    @FXML
    private TableColumn<Pair<Integer, Pair<Value, List<Value>>>, String> semaphoreListOfValuesColumn;
    ...
    public void refresh() {
        ...
        if (controller.getPrograms().size() > 0) {
            ...
            semaphore = controller.getPrograms().getFirst().getSemaphoreTable();
        }
        ...
        if (semaphore != null) {
            semaphore.getSemaphoreTable().forEach((k, v) -> semaphoreTable.getItems().add(new Pair<>(k, v)));
        }
        PrgState current;
        try {
    ...
    public void initialize() {
        ...
        this.semaphoreIndexColumn.setCellValueFactory(p -> new SimpleIntegerProperty(p.getValue().getKey()).asObject());
        this.semaphoreValueColumn.setCellValueFactory(p -> new SimpleObjectProperty<>(p.getValue().getValue().getKey().toString()));
        this.semaphoreListOfValuesColumn.setCellValueFactory(p -> new SimpleObjectProperty<>(p.getValue().getValue().getValue().toString()));
        this.refresh();
