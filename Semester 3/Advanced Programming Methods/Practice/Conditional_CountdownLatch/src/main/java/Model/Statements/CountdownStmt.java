        ...
        if (symTbl.isDefined(var)) {
            ...
            if (varValue.getType().equals(new IntType())) {
                ...
                if (latchTable.contains(foundIndex)) {
                    int val = latchTable.get(foundIndex);
                    if (val > 0) {
                        latchTable.put(foundIndex, val - 1);
                    }
                    state.getOut().add(new IntValue(state.getId()));
                } else {
                    throw new MyException("Countdown Stmt: The found index " + foundIndex + " does not exist in the latch table!");
                }
            } else {
                throw new MyException("Countdown Stmt: The variable " + var + " is not an integer!");
            }
        } else {
            throw new MyException("Countdown Stmt: The variable " + var + " is not defined!");
        }
        return null;
    }
    @Override
